	...
	public void convertMedia() throws IOException, InterruptedException {
		...
		while (!converterQueue.isEmpty()) {
			...
			try {
				...
				log.info("An exception occured while converting the media ["+pendingConversion.getMediaAsObject().getFileLocation()+"] due to ["+err.getMessage()+"]");
				conversionService.delete(pendingConversion);
			}
		}
	}
}
