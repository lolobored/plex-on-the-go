...
public class PlexServiceImpl implements PlexService {
	...
	private static final String plexUrlSignIn = "https://plex.tv/users/sign_in.json";
	private static final String plexUrlPmsServers = "https://plex.tv/pms/servers.xml";
	private static final String plexUrlServers = "https://plex.tv/servers/";
	...
	public String authenticate(String userName, String password) throws HttpException, IOException {
		...
		String response = HttpUtil.getInstance(false).postUrlEncoded(plexUrlSignIn, nameValuePairs, httpHeaders);
	...
	@Override
	public MediaContainer.Server retrieveMachineIdentifier(String token, String serverName) throws HttpException, JAXBException, PlexException {
		Map<String, String> httpHeaders = new HashMap();
		httpHeaders.put("X-Plex-Token", token);

		String response = HttpUtil.getInstance(false).get(plexUrlPmsServers, httpHeaders);
		JAXBContext jaxbContext = JAXBContext.newInstance(MediaContainer.class);
		Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();

		StringReader reader = new StringReader(response);
		List<MediaContainer.Server> servers = ((MediaContainer) unmarshaller.unmarshal(reader)).getServer();
		for (MediaContainer.Server server : servers) {
			if (serverName.equalsIgnoreCase(server.getName())) {
				return server;
			}
		}
		throw new PlexException("Unable to find the server name [" + serverName + "] in the list of servers");
	}

	@Override
	public AccessTokens retrieveAccessTokens(String token, String machineId) throws HttpException, JAXBException {
		Map<String, String> httpHeaders = new HashMap();
		httpHeaders.put("X-Plex-Token", token);

		String url = plexUrlServers + "/" + machineId + "/access_tokens.xml";
		String response = HttpUtil.getInstance(false).get(url, httpHeaders);
		JAXBContext jaxbContext = JAXBContext.newInstance(AccessTokens.class);
		Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();

		StringReader reader = new StringReader(response);
		return (AccessTokens) unmarshaller.unmarshal(reader);
